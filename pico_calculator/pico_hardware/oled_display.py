...
class OLEDDisplay:
    ...
    def update_input_three_label(self,title, label_one, label_two, label_three):
        ...
        self.oled.text(title, 0, 0)
        self.oled.hline(0, 10, 128, 1)
        self.oled.text(str(label_one), 0, 20)
        self.oled.text(str(label_two), 0, 40)
        self.oled.text(str(label_three), 0, 55)
        self.oled.show()

    def update_input_four_labels(self,title, label_one, label_two, label_three, label_four):
        self.oled.fill(0)
        self.oled.text(title, 0, 0)
        self.oled.hline(0, 10, 128, 1)
        self.oled.text(str(label_one), 0, 15)
        self.oled.text(str(label_two), 0, 26)
        self.oled.text(str(label_three), 0, 41)
        self.oled.text(str(label_four), 0, 55)
        self.oled.show()
